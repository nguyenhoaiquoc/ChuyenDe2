...
export class ProductService {
    ...
    async create(data: any) {
        const dealType = await this.dealTypeRepo.findOne({ where: { id: Number(data.deal_type_id) } });
        const condition = await this.conditionRepo.findOne({ where: { id: Number(data.condition_id) } });
        ...
        if (data.category_id && data.sub_category && data.sub_category.name) {
            ...
            if (existingSub) {
                ...
                console.log(`✅ Sử dụng sub_category tồn tại: ${data.sub_category.name} (id=${subCategoryId})`);
            } else {
                ...
                console.log(`✅ Tạo sub_category mới: ${data.sub_category.name} (${sourceTable})`);
            }
        ...
        if (subCategoryId) {
            ...
            if (subCategory) {
                ...
                if (!subCategory.source_table && sourceTable && subCategory.name) {
                    ...
                    console.log(`🔗 Đã cập nhật source_table (${sourceTable}) và source_id=${subCategory.source_id}`);
                }
            }
        }
        ...
        if (data.images && Array.isArray(data.images)) {
            ...
            if (imagesToSave.length > 0) {
                ...
                console.log(`⚠️ Không có ảnh hợp lệ để lưu (tất cả là file:// hoặc rỗng).`);
            }
        }
        ...
        return {
            ...
            tag: categoryName && subCategoryName
                ? `${categoryName} - ${subCategoryName}`
                : categoryName || subCategoryName || fullProduct.dealType?.name || 'Không có danh mục',
    ...
    async findAllFormatted(): Promise<any[]> {
        ...
        return products.map(p => {
            ...
            const sourceDetail = p.subCategory ? p.subCategory.source_table || null : null;
            ...
            return {
                ...
                // quan hệ chi tiết
                dealType: p.dealType ? { id: p.dealType.id, name: p.dealType.name } : null,
                condition: p.condition ? { id: p.condition.id, name: p.condition.name } : null,
                category: p.category ? { id: p.category.id, name: p.category.name, image: p.category.image, hot: p.category.hot } : null,
                ...
                tag: categoryName && subCategoryName ? `${categoryName} - ${subCategoryName}` : categoryName || subCategoryName || p.dealType?.name || 'Không có danh mục'
            }
        });
    }
    ...
    async getSourceDetail(subCategory: SubCategory): Promise<any> {
        ...
        switch (subCategory.source_table) {
            case 'fashion_categories':
                return await this.fashionRepo.findOne({ where: { id: subCategory.source_id } });
            case 'game_categories':
                return await this.gameRepo.findOne({ where: { id: subCategory.source_id } });
            case 'academic_categories':
                return await this.academicRepo.findOne({ where: { id: subCategory.source_id } });
            case 'animal_categories':
                return await this.animalRepo.findOne({ where: { id: subCategory.source_id } });
            case 'electronic_categories':
                return await this.electronicRepo.findOne({ where: { id: subCategory.source_id } });
            case 'house_categories':
                return await this.houseRepo.findOne({ where: { id: subCategory.source_id } });
            case 'vehicle_categories':
                return await this.vehicleRepo.findOne({ where: { id: subCategory.source_id } });
            default:
                return null;
        }
    }
    ...
    private getRepoByTable(tableName: string) {
        switch (tableName) {
            case 'fashion_categories': return this.dataSource.getRepository(FashionCategory);
            case 'game_categories': return this.dataSource.getRepository(GameCategory);
            case 'academic_categories': return this.dataSource.getRepository(AcademicCategory);
            case 'animal_categories': return this.dataSource.getRepository(AnimalCategory);
            case 'electronic_categories': return this.dataSource.getRepository(ElectronicCategory);
            case 'house_categories': return this.dataSource.getRepository(HouseCategory);
            case 'vehicle_categories': return this.dataSource.getRepository(VehicleCategory);
            default: throw new Error(`❌ Unknown source table: ${tableName}`);
        }
    }
