...
export default function ProductDetailScreen() {
    ...
    const productImages: ProductImage[] = product.images && product.images.length > 0
        ? product.images.map(img => ({
            ...
            image_url: img.image_url.startsWith('file://') || img.image_url.startsWith('http')
                ? img.image_url
        ...
        : [
            {
                ...
                name: "Default",
                image_url: product.image || "https://via.placeholder.com/400x300?text=No+Image",  // Thumbnail fallback
            ...
            <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
                ...
                <View className="relative">
                    ...
                    <TouchableOpacity className="absolute top-3 right-3 bg-white px-3 py-1 rounded-full flex-row items-center border border-gray-300">
                        <Ionicons name={product.isFavorite ? "heart" : "heart-outline"} size={16} color={product.isFavorite ? "red" : "black"} />
                ...
                <View className="px-4 py-3 pb-12">
                    ...
                    <Text className="text-red-600 text-xl font-bold mb-2">
                        {product.dealType?.name === "Miễn phí" ? "Miễn phí" :
                            product.dealType?.name === "Trao đổi" ? "Trao đổi" :
                                parseFloat(product.price || "0") > 0 ? `${parseFloat(product.price).toLocaleString()} đ` : null}
                    </Text>

                    {/* Địa chỉ */}
                    <Text className="text-gray-500 text-sm mb-1">
                        📍 {product.address_json?.full || product.location || "Chưa rõ địa chỉ"}
                    </Text>
                    ...
                    <View className="mb-6">
                        ...
                        <View className="border border-gray-200 rounded-lg">
                            ...
                            <View className="flex-row justify-between px-3 py-2 border-b border-gray-200">
                                ...
                                <Text className="text-gray-800 text-sm font-medium" style={{ flexShrink: 1, flexWrap: 'wrap' }}>
                            ...
                            {/* Danh mục trao đổi */}
                            {product.dealType?.name === "Trao đổi" && product.categoryChange && product.subCategoryChange && (
                                <View className="flex-row justify-between px-3 py-2 border-b border-gray-200">
                                    <Text className="text-gray-600 text-sm">Danh mục trao đổi</Text>
                                    <Text className="text-gray-800 text-sm font-medium" style={{ flexShrink: 1, flexWrap: 'wrap' }}>
                                        {product.categoryChange.name} - {product.subCategoryChange.name}
                                    </Text>
                                </View>
                            )}
